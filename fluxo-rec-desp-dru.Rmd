...
rec_RGPS_raw  <- read_excel("dados/Sankey Receitas por Fonte (com DRU) - RGPS.xlsx", skip = 10)
rec_nRGPS_raw <- read_excel("dados/Sankey Receitas por Fonte (com DRU) - exceto RGPS.xlsx", skip = 10)

rec_RGPS <- rec_RGPS_raw %>%
  mutate(classificador = "Contribuições e outras receitas do RGPS")

rec_nRGPS <- rec_nRGPS_raw %>%
  mutate(origem = str_sub(`Natureza Receita Código Completo`, 1, 3),
         classificador = case_when(
           origem %in% c("211", "212", "811") ~ "Emissões de títulos",
           origem %in% c("132", "164", "230") ~ "Juros e remunerações recebidas",
           origem %in% c("111", "711") ~ "Impostos",
           origem %in% c("121", "122", "721", "722") ~ "Contribuições sociais (exceto RGPS) e econômicas",
           origem == "134" ~ "Exploração de recursos naturais",
           origem == "292" ~ "Resultado do Banco Central",
           origem == "293" ~ "Remuneração da Conta Única",
          TRUE ~ "Demais receitas")) %>%
  select(-origem)

rec <- bind_rows(rec_nRGPS, rec_RGPS) %>%
  select(nr = classificador,
         nat_rec = `Natureza Receita Código Completo`,
         fte = `Fonte Recursos Código`,
         fte_nome = `Fonte Recursos Nome`,
         rec = `Saldo R$ (Item Informação)`)

# #verifica
# rec %>% group_by(nr) %>%
#   summarise(soma = sum(rec)) %>%
#   arrange(desc(soma))

# processa despesas ----------------------------------------------------------

des_raw <- read_excel("dados/Sankey Despesas por Fonte (com DRU).xlsx", skip = 10)

names(des_raw) <- c("fte", "fte_nome", "nd", "des")

des <- des_raw %>%
  mutate(des = as.numeric(des))

# dru ------------------------------------------------------------------------

exec_rec_dru <- read_excel("./dados/exec_rec_dru.xlsx", skip = 8)

lista_dru <- exec_rec_dru %>%
  ...
  select(nat_rec = Cod_Nat_Rec, fte = Cod_Fonte, DRU)

rec_dist_preliminar <- rec %>%
  left_join(lista_dru) %>%
  group_by(nr) %>%
  ...
  ungroup() %>%
  # gambiarra a partir daqui em razao da BAGUNÇA que fizeram
  # em 2019 nas classificações das receitas de contribuições
  mutate(DRU = ifelse(
    str_sub(nat_rec, 1, 4) %in% c("1211", "1212", "1213", "1214") & fte == "00",
    "DRU",
    DRU))

rec_dist <- rec_dist_preliminar %>%
  group_by(nr, DRU, fte, fte_nome, subtot_rec) %>%
  summarise(per_rec_fte = sum(per_rec_fte))

# ## verifica se percentuais estão ok
# rec_dist %>% group_by(nr) %>% summarise(soma = sum(per_rec_fte))
# rec_dist %>% group_by(nr) %>% summarise(soma = sum(per_rec_fte)) %>% group_by() %>% summarise(mean(soma) == 1)

# tirando a dru

rec_dist_sem_dru <- rec_dist %>% 
  filter(is.na(DRU)) %>%
  ungroup() %>%
  select(-DRU)
  
# processando a dru em separado
# (a dru vai ser receita e despesa)

rec_dist_so_dru <- rec_dist %>% 
  filter(DRU == "DRU") %>%
  select(-DRU)

dru_como_rec <- rec_dist_so_dru %>% 
  mutate(subtot_rec = subtot_rec * per_rec_fte) %>%
  group_by(fte, fte_nome) %>%
  summarise(subtot_rec = sum(subtot_rec)) %>%
  ungroup() %>%
  mutate(per_rec_fte = 1,
         nr = "DRU")
# ou seja, agora temos uma linha para a DRU como se fosse uma receita, em que 100% dos recursos são destinados à fonte 100.

# agora acrescentamos a DRU como receita/origem, para que sejam calculados seus ramos na matriz. depois de montarmos a matriz, vamos acrescentar a DRU como despesa/destino (pois suas origens já sabemos, está no rec_dist_so_dru)

rec_dist_final <- rec_dist_sem_dru %>%
  bind_rows(dru_como_rec)
...
dru_como_desp <- rec_dist_so_dru %>% 
  ungroup() %>%
  rename(p = per_rec_fte) %>%
  mutate(nd = "DRU",
         ramo = p * subtot_rec) %>%
  select(-DRU, -subtot_rec, -fte, -fte_nome)

# junta essa DRU à matriz:

matriz <- matriz_quase %>%
  bind_rows(dru_como_desp)

## testa
# matriz %>% group_by(nr) %>% summarise(sum(p))
```
...
rotulos <- unique(c(unique(matriz$nr),unique(matriz$nd)))
...
```{r message=FALSE, warning=FALSE}
p(matriz %>% filter(!(nd == "Amortização da Dívida" & nr == "Emissões de títulos") & !(nd == "Juros" & nr == "Emissões de títulos")), rotulos)
```
