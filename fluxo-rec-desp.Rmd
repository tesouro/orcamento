...
matriz <- matriz %>% 
  mutate(p = per_rec_fte * per_fte_des, # calcula o % de dist. de cada rec nas desps
         ramo = ifelse((round(subtot_rec * p,0)<1000000000),0,round(subtot_rec * p,0))) %>% # calcula então o tamanho de cada link, partir do subtotal de cada receita. ignorando se for menor que um bilhao
  group_by(nr,nd) %>%                        
  summarise(p = sum(p),
            ramo = sum(ramo)) %>%
  select(nr,nd,p,ramo)
...
verde_STN <- "#329c32"
cor_divida <- "#dd3127"
cor_divida_transluc <- "rgba(221, 49, 39,0.4)"
cor_RGPS <- azul_STN
cor_RGPS_transluc <- "rgba(0,75,147,0.4)"
...
matriz$cores_ramos <- "rgba(255,213,0,0.4)"
